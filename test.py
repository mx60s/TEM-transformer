...
model_spec = importlib.util.spec_from_file_location(
    "model", "../Summaries/" + date + "/run" + run + "/script/model.py"
)
...
params = torch.load(
    "../Summaries/" + date + "/run" + run + "/model/params_" + index + ".pt"
)
# Create a new tem model with the loaded parameters
tem = model.Model(params)
# Load the model weights after training
model_weights = torch.load(
    "../Summaries/" + date + "/run" + run + "/model/tem_" + index + ".pt"
)
...
model_input = [
    [[[walks[i][j][k]][0] for i in range(len(walks))] for k in range(3)]
...
plt.plot(
    analyse.smooth(
        np.mean(
            np.array(
                [env for env_i, env in enumerate(correct_model) if envs_to_avg[env_i]]
            ),
            0,
        )[1:],
        filt_size,
...
plt.plot(
    analyse.smooth(
        np.mean(
            np.array(
                [env for env_i, env in enumerate(correct_node) if envs_to_avg[env_i]]
            ),
            0,
        )[1:],
        filt_size,
...
plt.plot(
    analyse.smooth(
        np.mean(
            np.array(
                [env for env_i, env in enumerate(correct_edge) if envs_to_avg[env_i]]
            ),
            0,
        )[1:],
        filt_size,
...
plt.title(
    ...
    + str(
        np.mean(
            [np.mean(env) for env_i, env in enumerate(zero_shot) if envs_to_avg[env_i]]
        )
        * 100
    )
    + "%"
