...
def parameters():
    ...
    params["i_attractor_max_freq_inf"] = [
        params["i_attractor"] for _ in range(params["n_f"])
    ]
    ...
    params["i_attractor_max_freq_gen"] = [
        params["i_attractor"] - freq_nr for freq_nr in range(params["n_f_g"])
    ...
    params["p_update_mask"] = torch.zeros(
        (np.sum(params["n_p"]), np.sum(params["n_p"])), dtype=torch.float
    )
                ...
                if f_from > params["n_f_g"] and f_to > params["n_f_g"]:
                    if params["f_initial"][f_from] <= params["f_initial"][f_to]:
                        params["p_update_mask"][
                            n_p[f_from] : n_p[f_from + 1], n_p[f_to] : n_p[f_to + 1]
                        ] = 1.0
    ...
    params["p_retrieve_mask_inf"] = [
        torch.zeros(sum(params["n_p"])) for _ in range(params["i_attractor"])
    ]
    params["p_retrieve_mask_gen"] = [
        torch.zeros(sum(params["n_p"])) for _ in range(params["i_attractor"])
    ]
    ...
    params["g_connections"] = [
        [
            params["f_initial"][f_from] <= params["f_initial"][f_to]
            for f_from in range(params["n_f_g"])
        ]
    ...
    params["g_connections"] = params["g_connections"] + [
        ...
        + [
            params["f_initial"][f_from] <= params["f_initial"][f_to]
            for f_from in range(params["n_f_g"], params["n_f"])
        ]
    ...
    params["W_repeat"] = [
        torch.tensor(
            np.kron(
                np.eye(params["n_g_subsampled"][f]), np.ones((1, params["n_x_f"][f]))
            ),
            dtype=torch.float,
        )
    ...
    params["W_tile"] = [
        torch.tensor(
            np.kron(
                np.ones((1, params["n_g_subsampled"][f])), np.eye(params["n_x_f"][f])
            ),
            dtype=torch.float,
        )
        for f in range(params["n_f"])
...
def parameter_iteration(iteration, params):
    ...
    p2g_scale_offset = 1 / (
        1 + np.exp((iteration - params["p2g_sig_half_it"]) / params["p2g_sig_scale_it"])
    )
    # Calculate current learning rate
    lr = max(
        params["lr_min"]
        + (params["lr_max"] - params["lr_min"])
        * (params["lr_decay_rate"] ** (iteration / params["lr_decay_steps"])),
        params["lr_min"],
    )
    ...
    L_p_g = (
        min((iteration + 1) / params["loss_weights_p_g_it"], 1)
        * params["loss_weights_p"]
    )
    L_p_x = (
        min((iteration + 1) / params["loss_weights_p_g_it"], 1)
    ...
    L_reg_g = (1 - min((iteration + 1) / params["loss_weights_reg_g_it"], 1)) * params[
        "loss_weights_reg_g"
    ]
    L_reg_p = (1 - min((iteration + 1) / params["loss_weights_reg_p_it"], 1)) * params[
        "loss_weights_reg_p"
    ]
