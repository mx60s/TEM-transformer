...
if load_existing_model:
    ...
    (
        ...
        script_path,
        envs_path,
    ) = utils.set_directories(date, run)
...
walks = [
    env.generate_walks(
        params["n_rollout"]
        * np.random.randint(params["walk_it_min"], params["walk_it_max"]),
        1,
    )[0]
...
for i in range(i_start, params["train_it"]):
    ...
    for env_i, walk in enumerate(walks):
        ...
        if len(walk) < params["n_rollout"]:
            ...
            environments[env_i] = world.World(
            ...
            # Log progress
            logger.info(
                "Iteration {:d}: new walk of length {:d} for batch entry {:d}".format(
                    i, len(walk), env_i
                )
            )
