...
# each location has an observation (int) and a set of actions associated with it
# each action has an id, a transition vector, and a probability
...
class Step:
    ...
    def __init__(self, location={}, observation=0, action={}):
...
class Walk(UserList):
    """
    A list of steps taken
    """

    def __init__(self, walk=[]):
        super().__init__(step for step in walk)

    def get_locations(self) -> List:
        return [step.location for step in self.data]

    def get_observations(self) -> List:
        return [step.observations for step in self.data]

    def get_actions(self) -> List:
        return [step.actions for step in self.data]

    def __setitem__(self, index, step):
        self.data[index] = step
        # super().__setitem__(index, str(item))

    def insert(self, index, step):
        self.data.insert(index, step)
...
class World:
    """
    The basic setup of a world that a rat would run around in, with possible policies and setups.
    """
    ...
    def generate_walks(self, walk_length=10, n_walk=100, repeat_bias_factor=2) -> List:
        """
        Generate walk by sampling actions accoring to policy, then next state according to graph
        Returns: list of state observation action tuples
        """
        walks = []
        for _ in range(n_walk):
            ...
            for step in new_walk[:-1]:
                step.location = {
                    "id": step.location["id"],
                    "shiny": step.location["shiny"],
                }
            # Append new walk to list of walks
            walks.append(new_walk)
        return walks

    def walk_default(self, walk, walk_length, repeat_bias_factor=2) -> Tuple:
        """
        ...
        for _ in range(walk_length - len(walk)):
            ...
            walk.append(Step(new_location, new_observation, new_action))
        # Return the final walk
        return walk

    def walk_shiny(self, walk, walk_length, repeat_bias_factor=2) -> Tuple:
        """
        Conduct a shiny walk
        """
    ...
    def get_location(self, walk: Walk) -> Dict:
        ...
        # Any other step: get new location from previous location and action
            ...
            new_location = int(
                np.flatnonzero(
                    np.cumsum(
                        walk[-1].location["actions"][walk[-1].action]["transition"]
                    )
                    > np.random.rand()
                )[0]
            )
    ...
    def get_action(self, new_location: Dict, walk: Walk, repeat_bias_factor=2) -> int:
        """
        Get the next action given current location and general policy
        """
        ...
        policy[
            []
            if len(walk) == 0 or new_location["id"] == walk[-1].location["id"]
            else walk[-1].action
        ] *= repeat_bias_factor
    ...
    # really all this does is make it so you can't just stay in the reward location
    # can't you just turn around again?
    # can you just "consume" the reward though
    def get_reward(self, reward_locations) -> Tuple[List, List]:
        """
        ...
        reward_locations = (
            [reward_locations]
            if not isinstance(reward_locations, list)
